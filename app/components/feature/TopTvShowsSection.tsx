...
interface TopTvShowsSectionProps {
  tvShows: Array<{
    id: number;
    name: string;
    poster_path: string;
...
export default function TopTvShowsSection({ tvShows }: TopTvShowsSectionProps) {
  return (
    <section className="py-16 bg-gradient-to-b from-background/50 to-background">
      <div className="container mx-auto px-4 max-w-7xl">
        {/* Section Header */}
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-4xl font-bold text-white">TOP 10 Shows</h2>
          <Link
            href="/tv"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
          {tvShows.slice(0, 10).map((show, index) => (
            <Link
              ...
              <div className="relative aspect-[2/3] rounded-lg overflow-hidden bg-zinc-800">
                ...
                <div className="absolute top-2 right-2 z-10">
                  <div className="flex items-center gap-1 bg-black/70 backdrop-blur-sm px-2 py-1 rounded-md">
                    ...
                    <span className="text-white text-xs font-bold">{show.vote_average?.toFixed(1) || 'N/A'}</span>
                  </div>
                </div>
              </div>
