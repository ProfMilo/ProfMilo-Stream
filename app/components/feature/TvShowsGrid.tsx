...
import { Star, Calendar, Clock, TrendingUp, MonitorPlay, ChevronLeft, ChevronRight } from 'lucide-react';
...
const filters = [
    ...
    { id: 'airing_today', label: 'Airing Today', icon: Calendar, description: 'Airing on TV today' },
    { id: 'top_rated', label: 'Top Rated', icon: TrendingUp, description: 'Highest rated of all time' },
    { id: 'on_the_air', label: 'On the Air', icon: Clock, description: 'Currently airing shows' },
];

const API_KEY = process.env.NEXT_PUBLIC_TMDB_API_KEY;
const BASE_URL = 'https://api.themoviedb.org/3';

export default function TvShowsGrid({ tvShows: initialTvShows }: TvShowsGridProps) {
        ...
        async function fetchTvShows() {
            ...
            try {
                const res = await fetch(
                    `${BASE_URL}/tv/${activeFilter}?api_key=${API_KEY}&page=${currentPage}`
                );
                if (res.ok) {
                    const data = await res.json();
                    setTvShows(data.results.map((t: any) => ({ ...t, media_type: 'tv' })));
        ...
        <section className="py-8 bg-background">
            <div className="container mx-auto px-4 max-w-7xl">
                ...
                <div ref={sectionRef} className="flex items-center gap-3 mb-8">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-white">{currentFilterInfo.label} TV Shows</h2>
